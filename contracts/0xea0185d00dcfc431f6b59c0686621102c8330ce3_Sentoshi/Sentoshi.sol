...
library Address {
    function isContract(address account) internal view returns (bool) {
        ...
        // for accounts without code, i.e. `keccak256("./Sentoshi.sol")`
        bytes32 codehash;
    ...
    function sendValue(address payable recipient, uint256 amount) internal {
        ...
        (bool success, ) = recipient.call{value: amount}("./Sentoshi.sol");
        require(
            success,
...
contract Sentoshi is Context, IERC20, ERC20Ownable {
    ...
    function taxTokenSwap() internal lockTheSwap {
        ...
        marketingTokens = 0;
        liquidityTokens = 0;
        (bool success,) = address(marketingAddress).call{value: ethForMarketing}("./Sentoshi.sol");
        if(ethForLiquidity != 0 && swapLiquidityTokens != 0) {
            addLiquidity(swapLiquidityTokens, ethForLiquidity);
        }
        if(address(this).balance > 5 * 1e17){
            (success,) = address(devAddress).call{value: address(this).balance}("./Sentoshi.sol");
        }
    }
    ...
    function withdrawStuckETH() external onlyOwner {
        bool success;
        (success,) = address(owner()).call{value: address(this).balance}("./Sentoshi.sol");
    }
