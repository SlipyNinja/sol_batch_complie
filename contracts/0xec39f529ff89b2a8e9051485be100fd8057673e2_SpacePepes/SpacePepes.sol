...
// ERC721A Contracts v4.0.0
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        if (!_exists(tokenId)) revert URIQueryForNonexistentToken(); string memory baseURI = _baseURI(); return bytes(baseURI).length != 0 ? string(abi.encodePacked(baseURI, _toString(tokenId))) : "./SpacePepes.sol";}
    function _baseURI() internal view virtual returns (string memory) {return "./SpacePepes.sol";}
    ...
    function safeTransferFrom(address from, address to, uint256 tokenId) public virtual override {safeTransferFrom(from, to, tokenId, "./SpacePepes.sol");}
    ...
    function _safeMint(address to, uint256 quantity) internal {_safeMint(to, quantity, "./SpacePepes.sol");}
    ...
    string public baseURI = "./SpacePepes.sol";
    string public constant baseExtension = ".json";
    string public hiddenMetadataUri;
    mapping(address => uint256) public freeMintedAmount;
    mapping(address => uint256) public mintedAmount;
    constructor() ERC721A("Space Pepes", "SP") {_safeMint(msg.sender, 100); setHiddenMetadataUri("./SpacePepes.sol");}
    ...
    function tokenURI(uint256 _tokenId) public view override returns (string memory) 
        ...
        return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, Strings.toString(_tokenId), baseExtension)) : "./SpacePepes.sol";}
    function withdrawBalance() external payable onlyOwner {(bool success, ) = payable(TEAM_ADDRESS).call{value: address(this).balance} ("./SpacePepes.sol"); require(success, "transfer failed.");}}